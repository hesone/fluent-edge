 ...
 *   { type: "tts_audio", sampleRate } followed immediately by one binary
 *   frame containing raw 16-bit PCM at that rate.
 *
...
import { synthesise, TTS_SAMPLE_RATE } from "./tts-engine.js";
...
async function handleTTSRequest(ws, msg) {
  try {
    ...
    // Announce the rate rather than letting the client guess: Piper "-medium"
    // voices are 22050 Hz but "-low"/"-x_low" are 16000 Hz, and decoding at the
    // wrong rate plays the audio fast and sharp.
    ws.send(JSON.stringify({ type: "tts_audio", sampleRate: TTS_SAMPLE_RATE }));
